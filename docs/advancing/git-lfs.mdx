...
The Git LFS client communicates with the Gogs server over HTTP/HTTPS or SSH. Over HTTP, it uses HTTP Basic Authentication to authorize client requests. Over SSH, the Git LFS client uses the pure SSH transfer protocol (`git-lfs-transfer`) to upload and download objects directly through the SSH connection, so no additional HTTP credentials are needed.
...
<AccordionGroup>
  ...
  <Accordion title="SSH LFS transfer requires Git LFS 3.0+">
    The pure SSH transfer protocol (`git-lfs-transfer`) requires Git LFS client version 3.0 or later. Older clients will fall back to HTTP/HTTPS for LFS transfers even when using an SSH remote.
  </Accordion>
